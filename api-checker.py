...
class Window(Frame):
	...
	def init_window(self):
		...
		helv12 = tkFont.Font(family='verdana', size=12, weight='bold')
		...
		self.lb_tasks = Listbox(lf, bg="white", width=65, height=17)
		self.lb_tasks.grid(row=0, column=0)

		# BOTTOM LEFT PANEL LISTBOX
		self.lb_tasksb = Listbox(lf, bg="white", width=65, height=17)
		self.lb_tasksb.grid(row=1, column=0)
				
		# LEFT PANEL TOP SCROLLBAR
		self.sb1 = Scrollbar(lf)
		self.sb1.grid(row=0,column=1, ipady=166, sticky=N)
		...
		self.sb2.grid(row=1,column=1, ipady=164, sticky=N)
		...
		self.lb_tasksbt = Listbox(rf, bg="white", width=44, height=10, selectmode=SINGLE)
		self.lb_tasksbt.pack(side=LEFT, fill=X, expand=0)

		# RIGHT PANEL BOTTOM SCROLLBAR
		sb3 = Scrollbar(rf)
		sb3.pack(side=RIGHT, ipady=91)
